...
function GestionPaiements({ paiementConfig, savePaiementConfig }){
  ...
  function save(){
    savePaiementConfig({ flooz: flooz.trim(), tmoney: tmoney.trim() });
    setSaved(true);
...
function GerantDashboard({ commandes,setCommandes,upsertCmd,friperie,setFriperie,tarifs,setTarifs,rewards,setRewards,livreurs,setLivreurs,gerantPin,setGerantPin,adminPw,setAdminPw,clients,setClients,onLogout }){
  ...
  React.useEffect(()=>{
    ...
    if(nouvelles.length>0) setLivNotif(n=>[...n,...nouvelles]);
...
export default function App(){
  ...
  const [paiementConfig, savePaiementConfig, paiReady] = useFireDoc("config","paiements", {flooz:JOKER_FLOOZ_DEFAULT, tmoney:JOKER_TMONEY_DEFAULT});
  ...
  const JOKER_FLOOZ = paiementConfig?.flooz||JOKER_FLOOZ_DEFAULT;
  const JOKER_TMONEY = paiementConfig?.tmoney||JOKER_TMONEY_DEFAULT;
