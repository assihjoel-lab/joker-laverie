...
function NouvelleCommande({ commandes,setCommandes,upsertCmd,clients,upsertClient,tarifs,promos,employeActif,produits,onAutoConsume,onBack,onCreated,onDone,template }){
  ...
  // Réduction parrainage : 10% si nouveau client avec code parrain valide
  const estNouveauClient = !(clients||[]).find(cl=>cl.nom?.toLowerCase()===nom.trim().toLowerCase()||(tel&&cl.tel===tel.trim()));
  const remiseParrainage = (parrainValide&&estNouveauClient) ? Math.round((sousTotal+frais-remiseMontant)*10/100) : 0;
  const total=sousTotal+frais-remiseMontant-remiseParrainage;
  const pts=Math.floor(total/100);

  function verifierParrain(){
    const code=codeParrain.trim().toUpperCase();
    if(!code){setParrainValide(null);return;}
    const parrain=(clients||[]).find(cl=>cl.codeClient===code);
    setParrainValide(parrain?parrain:false);
  }
    ...
    if(upsertClient && nom.trim()) {
      ...
      if(existingCli) {
        upsertClient({
          ...existingCli,
          codeClient,
          points: (existingCli.points||0) + pts,
          historique:[
            cmdEntry,
            {date:c.date, pts, desc:`Commande ${c.id}`},
            ...(existingCli.historique||[])
          ],
          totalDepense:(existingCli.totalDepense||0)+c.total,
          commandes:[cmdEntry,...(existingCli.commandes||[])]
        });
      } else {
        ...
        if(parrainValide&&parrainValide.id){
          ...
          if(parrainValide.tel&&parrainValide.tel!=="—"){
            setTimeout(()=>sendWhatsApp(parrainValide.tel,`🃏 *JOKER Laverie & Service*\n\n🎉 Merci pour votre parrainage !\n\n${nom.trim()} a utilisé votre code et a effectué sa première commande.\n\n🏅 +20 points fidélité ajoutés à votre compte !`),600);
          }
        }
      }
    }
    ...
    <div style={{padding:"24px 20px",animation:"fadeIn 0.4s ease"}}>
      ...
      <div style={{background:CARD,borderRadius:18,padding:20,border:`1px solid ${BDR}`,marginBottom:16}}>
        {(()=>{
          ...
          const fraisT = ticket.livraison?(ticket.fraisLiv||LIVRAISON_TARIF_DEFAULT):0;
  ...
  return (
    <div style={{padding:"20px 20px 0",animation:"fadeIn 0.4s ease"}}>
      ...
      <div style={{marginBottom:14}}>
        <label style={{fontSize:11,color:"#8892B0",letterSpacing:1,textTransform:"uppercase",display:"block",marginBottom:8}}>🎁 Code de parrainage (optionnel)</label>
        <div style={{display:"flex",gap:8}}>
          <input value={codeParrain} onChange={e=>{setCodeParrain(e.target.value.toUpperCase());setParrainValide(null);}} placeholder="CLI-XXXX"
            style={{flex:1,background:CARD,border:`1px solid ${BDR}`,borderRadius:12,padding:"11px",color:"#F8FAFF",fontSize:14,outline:"none"}} />
          <button onClick={verifierParrain} disabled={!codeParrain.trim()} style={{background:codeParrain.trim()?`linear-gradient(135deg,${BLU},${BLU2})`:CARD,border:"none",borderRadius:12,padding:"11px 16px",color:"#fff",fontWeight:700,cursor:codeParrain.trim()?"pointer":"default"}}>Vérifier</button>
        </div>
        {parrainValide===false&&<p style={{color:"#FF4444",fontSize:12,marginTop:6}}>❌ Code introuvable</p>}
        {parrainValide&&parrainValide.id&&(
      ...
      {sousTotal>0&&(
        <div style={{background:`linear-gradient(135deg,#0D1F6E,#1A3EBD22)`,borderRadius:18,padding:18,marginBottom:16,border:`1px solid ${isDepot?"rgba(255,184,0,0.3)":"rgba(0,194,255,0.3)"}`}}>
          ...
          {remiseParrainage>0&&(
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
              <span style={{color:"#A855F7",fontSize:13}}>🎁 Parrainage (10%)</span>
              <span style={{color:"#A855F7",fontWeight:700,fontSize:13}}>-{fmt(remiseParrainage)} FCFA</span>
            </div>
          )}
...
function TicketModal({ c, tarifs, onClose }){
  ...
  const frais = c.livraison ? LIVRAISON_TARIF_DEFAULT : 0;
    ...
    <div style={{background:CARD,borderRadius:20,padding:16,marginBottom:12,border:`1px solid ${needsAttention?"#FF6B35":c.poidsStatut==="estimated"?"rgba(255,184,0,0.25)":BDR}`,position:"relative"}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
        <div style={{flex:1}}>
          ...
          {c.livraison&&<span style={{fontSize:11,color:"#A855F7",marginLeft:6}}>🛵 {fmt(c.livraisonFrais||LIVRAISON_TARIF_DEFAULT)}F</span>}
        </div>
        ...
        {/* Bouton Payer — toujours visible si pas encore payé */}
        {!c.paiementConfirme&&<button onClick={onPay} style={{background:`linear-gradient(135deg,#004d20,${BLU})`,border:"1px solid rgba(0,166,81,0.3)",borderRadius:10,color:"#fff",padding:"7px 12px",fontSize:12,fontWeight:700,cursor:"pointer"}}>💳 Payer</button>}
        ...
        {c.livraison&&c.livraisonStatut==="confirmed"&&c.statut==="Prêt"&&(
          <button onClick={onNext} style={{background:"linear-gradient(135deg,#004d20,#006b2b)",border:"1px solid #4ADE8040",borderRadius:10,color:"#4ADE80",padding:"7px 12px",fontSize:12,fontWeight:700,cursor:"pointer"}}>📦 Linge remis</button>
        )}
      </div>
      {editFrais&&(
...
function EditCmdModal({ c, tarifs, clients, upsertClient, onClose, onSave }){
  const tarifsDisp = tarifs&&tarifs.length>0 ? tarifs : TARIFS_INIT;
  ...
  const ancienTotal = c.total||0;
  const ancienPts   = c.points||0;

  function addService(tid){
    const t=tarifsDisp.find(x=>x.id===tid); if(!t) return;
    setPanier(p=>[...p,{tarifId:tid,poids:"",qte:1,isKg:(t.type||"kg")==="kg",note:""}]);
  }
  function removeService(idx){ setPanier(p=>p.filter((_,i)=>i!==idx)); }
  function updateService(idx,field,val){ setPanier(p=>p.map((s,i)=>i===idx?{...s,[field]:val}:s)); }
  ...
  const frais      = c.livraison?(c.fraisLiv||LIVRAISON_TARIF_DEFAULT):0;
  const remiseMont = Math.round((sousTotal+frais)*remisePct/100);
  const total      = sousTotal+frais-remiseMont;
  const nouveauxPts = Math.floor(total/100);
  const diffPts     = nouveauxPts - ancienPts;
  ...
  function sauvegarder(){
    ...
    const updatedCmd = {
      ...
      panier: panier.map(s=>{
        ...
        return {tarifId:s.tarifId,label:t.label,prix:t.prix,type:s.isKg?"kg":"unite",
          poids:s.isKg?(parseFloat(s.poids)||0):0,qte:s.isKg?1:(parseInt(s.qte)||1),note:s.note||""};
      }),
      poids:poidsTotal, tarifId:firstTarif.id, tarif:firstTarif.prix,
      total, remise:remisePct, points:nouveauxPts, note,
    };

    // ── Recalcul des points du client ──
    if(upsertClient && c.codeClient) {
      const cli = (clients||[]).find(cl=>cl.codeClient===c.codeClient||cl.nom?.toLowerCase()===c.client?.toLowerCase());
      if(cli && diffPts!==0) {
        const newPts = Math.max(0,(cli.points||0)+diffPts);
        upsertClient({
          ...cli,
          points: newPts,
          historique:[
            {date:todayStr(), pts:diffPts, desc:`Modification commande ${c.id} (${diffPts>0?"+":""}${diffPts} pts)`},
            ...(cli.historique||[])
          ]
        });
      }
      // Mettre à jour le totalDepense du client
      if(cli && ancienTotal!==total) {
        const newTotal = Math.max(0,(cli.totalDepense||0)+(total-ancienTotal));
        upsertClient({...cli, totalDepense:newTotal});
      }
    }
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:9999,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-start",padding:"16px 12px",overflowY:"auto"}}
      ...
      <div style={{width:"100%",maxWidth:420,background:"#0D1629",borderRadius:24,padding:"20px 18px 30px",border:`1px solid ${BLU2}40`}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:14}}>
          {[0,5,10,15,20].map(p=>(
            <button key={p} onClick={()=>setRemisePct(p)}
              style={{flex:1,background:remisePct===p?"linear-gradient(135deg,#4ADE80,#22C55E)":DARK,border:`1px solid ${remisePct===p?"#4ADE80":BDR}`,borderRadius:10,padding:"8px 4px",color:remisePct===p?"#000":"#8892B0",fontWeight:remisePct===p?700:400,fontSize:12,cursor:"pointer"}}>
              {p===0?"Aucune":`-${p}%`}
            </button>
          ))}
        </div>

        {/* Récap total + impact points */}
        {sousTotal>0&&(
          <div style={{background:`linear-gradient(135deg,#0D1F6E,#1A3EBD22)`,borderRadius:16,padding:16,marginBottom:16,border:"1px solid rgba(0,194,255,0.3)"}}>
            {(frais>0||remiseMont>0)&&<>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{color:"#8892B0",fontSize:13}}>Services</span><span>{fmt(sousTotal)} F</span></div>
              ...
              {remiseMont>0&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{color:"#4ADE80",fontSize:13}}>Réduction ({remisePct}%)</span><span style={{color:"#4ADE80",fontWeight:700}}>-{fmt(remiseMont)} F</span></div>}
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontSize:12,color:"#8892B0"}}>Impact points client</span>
              <span style={{fontSize:13,fontWeight:700,color:diffPts>=0?"#4ADE80":"#FF6B6B"}}>
                {diffPts===0?"Inchangé":diffPts>0?`+${diffPts} pts`:diffPts+" pts"}
              </span>
            </div>
          </div>
        )}
        ...
        <div style={{display:"flex",gap:10}}>
          ...
          <button onClick={sauvegarder} disabled={panier.length===0||sousTotal===0}
            style={{flex:2,background:panier.length>0&&sousTotal>0?`linear-gradient(135deg,${BLU},${BLU2})`:DARK,border:"none",borderRadius:14,padding:"13px",color:panier.length>0&&sousTotal>0?"#fff":"#8892B0",fontWeight:700,fontSize:15,cursor:panier.length>0&&sousTotal>0?"pointer":"not-allowed"}}>
            💾 Sauvegarder
          </button>
        </div>
      </div>
...
useEffect(()=>{
  ...
  function logAction(type, detail) {
    ...
    upsertJournal({
      id: "j_" + Date.now(),
      employeId:  employeActif.id,
      employeNom: employeActif.nom,
      type,
      detail,
      date: todayStr(),
  ...
  const tabs=[
    ...
    ...(employeActif?[]:[{id:"plus",icon:"⚙️",label:"Plus"}]),
  ];
  ...
  return (
    ...
    {editCmd&&<EditCmdModal c={editCmd} tarifs={tarifs} clients={clients} upsertClient={upsertClient} onClose={()=>setEditCmd(null)} onSave={updated=>{setCommandes(p=>p.map(c=>c.id===updated.id?updated:c));upsertCmd(updated);logAction("modification",`${updated.client} · ${updated.id} · ${fmt(updated.total)} F`);setEditCmd(null);}} />}
    {payCmd&&<PayModal c={payCmd} onClose={()=>setPayCmd(null)} onConfirm={(m)=>confirmerPaiement(payCmd.id,m)} />}
      ...
      <div style={{position:"sticky",top:0,zIndex:50,background:"rgba(6,13,31,0.97)",backdropFilter:"blur(20px)",borderBottom:`1px solid ${BDR}`,padding:"10px 16px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <Logo size={30} />
        {employeActif
          ? <div style={{display:"flex",alignItems:"center",gap:6,background:"#0D1F6E22",borderRadius:20,padding:"4px 10px",border:"1px solid #1A3EBD44"}}>
              <span style={{fontSize:16}}>{employeActif.emoji||"👤"}</span>
              <span style={{fontFamily:"'Bebas Neue',cursive",fontSize:13,letterSpacing:1,color:"#4A7BF7"}}>{employeActif.nom}</span>
            </div>
      ...
      {tab==="plus"&&(
        <div style={{padding:"20px 20px 0",animation:"fadeIn 0.4s ease"}}>
          ...
          {[
            ...
            {id:"journal",   icon:"📋", label:"Journal des actions",      sub:"Traçabilité par employé"},
          ].map(m=>(
      ...
      {tab==="employes"&&(
        <div><button onClick={()=>setTab("plus")} style={{background:"none",border:"none",color:BLU2,cursor:"pointer",padding:"16px 20px",fontSize:14}}>← Retour</button><GestionEmployes employes={employes||[]} upsertEmploye={upsertEmploye} removeEmploye={removeEmploye} /></div>
      )}
      {tab==="journal"&&(
...
function ChoixEmploye({ employes, onChoix, onGerant }) {
  ...
  if (sel) return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:DARK,padding:24}}>
      <div style={{width:"100%",maxWidth:340,animation:"fadeIn 0.3s ease"}}>
        <button onClick={()=>{setSel(null);setPin("");setErr(false);}} style={{background:"none",border:"none",color:BLU2,cursor:"pointer",fontSize:14,marginBottom:20}}>← Changer d'employé</button>
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{width:72,height:72,borderRadius:20,background:`${BLU}40`,border:`2px solid ${BLU2}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,margin:"0 auto 12px"}}>{sel.emoji||"👤"}</div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:16}}>
            {employes.map(e=>(
              <button key={e.id} onClick={()=>setSel(e)}
                ...
                <span style={{color:"#8892B0",marginLeft:"auto",fontSize:20}}>›</span>
              </button>
            ))}
          </div>
        )}
...
function GestionEmployes({ employes, upsertEmploye, removeEmploye }) {
  ...
  return (
    <div style={{padding:"20px 20px 0",animation:"fadeIn 0.4s ease"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <h2 style={{fontFamily:"'Bebas Neue',cursive",fontSize:26,letterSpacing:2}}>👥 Employés</h2>
        <button onClick={()=>{setShowAdd(s=>!s);setEditing(null);setForm({nom:"",pin:"",role:"Employé",emoji:"👤"});}}
          style={{background:showAdd?"#1A0A0A":`linear-gradient(135deg,${BLU},${BLU2})`,border:"none",borderRadius:12,padding:"8px 16px",color:showAdd?"#8892B0":"#fff",fontWeight:700,fontSize:13,cursor:"pointer"}}>
      ...
      {showAdd&&(
        <div style={{background:CARD,borderRadius:18,padding:18,marginBottom:14,border:`1px solid ${BLU2}40`,animation:"fadeIn 0.2s ease"}}>
          ...
          <select value={form.role} onChange={e=>setForm(p=>({...p,role:e.target.value}))} style={{...inp,marginBottom:14}}>
            {ROLES.map(r=><option key={r} value={r}>{r}</option>)}
          </select>
          <div style={{display:"flex",gap:8}}>
            <button onClick={save} style={{flex:1,background:`linear-gradient(135deg,${BLU},${BLU2})`,border:"none",borderRadius:12,padding:"13px",color:"#fff",fontWeight:700,cursor:"pointer"}}>💾 {editing?"Mettre à jour":"Enregistrer"}</button>
            <button onClick={()=>{setShowAdd(false);setEditing(null);}} style={{background:DARK,border:`1px solid ${BDR}`,borderRadius:12,padding:"13px 16px",color:"#8892B0",cursor:"pointer"}}>✕</button>
          </div>
        </div>
      )}
      ...
      {employes.map(e=>(
        <div key={e.id} style={{background:CARD,borderRadius:18,padding:"14px 16px",marginBottom:10,border:`1px solid ${BDR}`,display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{flex:1}}>
            <p style={{fontWeight:700,fontSize:15}}>{e.nom}</p>
            <p style={{fontSize:12,color:"#8892B0",marginTop:2}}>{e.role||"Employé"} · PIN : {"●".repeat(4)}</p>
          </div>
          <button onClick={()=>startEdit(e)} style={{background:"none",border:`1px solid ${BDR}`,borderRadius:10,padding:"6px 10px",color:BLU2,fontSize:12,cursor:"pointer"}}>✏️</button>
          <button onClick={()=>removeEmploye(String(e.id))} style={{background:"none",border:"none",color:"#FF4444",fontSize:18,cursor:"pointer"}}>🗑️</button>
        </div>
      ))}
    </div>
  );
...
function JournalActions({ journal, employes }) {
  ...
  journal.forEach(j=>{
    ...
    stats[j.employeId][j.type]=(stats[j.employeId][j.type]||0)+1;
  });
  ...
  return (
    <div style={{padding:"20px 20px 0",animation:"fadeIn 0.4s ease"}}>
      ...
      <div style={{display:"flex",gap:6,marginBottom:10,overflowX:"auto",paddingBottom:4}}>
        ...
        {employes.map(e=>(
          <button key={e.id} onClick={()=>setFiltreEmp(e.id)} style={{flexShrink:0,background:filtreEmp===e.id?`linear-gradient(135deg,${BLU},${BLU2})`:CARD,border:`1px solid ${filtreEmp===e.id?BLU2:BDR}`,borderRadius:20,padding:"6px 12px",color:filtreEmp===e.id?"#fff":"#8892B0",fontSize:11,fontWeight:700,cursor:"pointer"}}>{e.emoji||"👤"} {e.nom}</button>
        ))}
      </div>
      <div style={{display:"flex",gap:6,marginBottom:14,overflowX:"auto",paddingBottom:4}}>
        {[["tous","Tout","#8892B0"],["commande","➕ Cmd","#4ADE80"],["paiement","💳 Pmt",CYAN],["statut","🔄 Statut",BLU2],["suppression","🗑️ Suppr","#FF6B6B"]].map(([v,l,c])=>(
        ...
        return (
          <div key={j.id} style={{background:CARD,borderRadius:14,padding:"12px 14px",marginBottom:8,border:`1px solid ${t.c}20`,display:"flex",gap:12,alignItems:"flex-start"}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <p style={{fontSize:12,color:"#8892B0",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{j.detail}</p>
            </div>
          </div>
        );
      })}
...
export default function App(){
 ...
 if(!user && !showLogin && !employeActifApp) return (
  ...
  if(!user && !employeActifApp) return <LoginScreen onBack={()=>setShowLogin(false)} />;
